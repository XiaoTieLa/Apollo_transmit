...
import { BinaryOpcode, ClientBinaryOpcode, FetchAssetStatus, ServerCapability, } from ".";
...
async function setupServerAndClient(server) {
    ...
    const msgQueue = new ConsumerQueue();
    ...
    const eventQueue = new ConsumerQueue();
...
describe("FoxgloveServer", () => {
    ...
    it("handles client subscribe/unsubscribe and forwards messages", async () => {
        ...
        try {
            ...
            server.sendMessage(chanId, 42n, new Uint8Array([1, 2, 3]));
            await expect(nextBinaryMessage()).resolves.toEqual(new Uint8Array([BinaryOpcode.MESSAGE_DATA, ...uint32LE(subId), ...uint64LE(42n), 1, 2, 3]));
    ...
    it("sends time messages to clients", async () => {
        ...
        try {
            ...
            await expect(nextBinaryMessage()).resolves.toEqual(new Uint8Array([BinaryOpcode.TIME, ...uint64LE(42n)]));
        }
        finally {
            close();
        }
    ...
    it.each([
        ...
        try {
            ...
            const data = response.status === FetchAssetStatus.SUCCESS
                ? response.data
